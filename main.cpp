...
//Dados que vão para o painel do carro
void displayData(uint16_t vel, uint16_t Hz, uint16_t temp, uint16_t comb, uint16_t tempCVT, uint16_t SOC);
// Velocidade, RPM do motor, Temperatura do motor, Nivel de combustivel, temperatura da CVT, Estado da Bateria
...
uint8_t switch_state = 0x00, flags = 0x00, pulse_counter = 0, temp_motor = 0,soc = 0, temp_cvt = 0 ;
...
//                serial.printf("rpm_hz=%d\r\n", rpm_hz);
                displayData(speed_display, rpm_hz, temp_motor, (flags & 0x08), temp_cvt, soc);
...
void filterMessage(CANMsg msg)
{
    ...
    if(msg.id == RPM_ID)
 {
    ...
    {
        //alterar aqui, retirar a flag e colocar uma variavel uint16_t/
        msg >> flags;
    }
